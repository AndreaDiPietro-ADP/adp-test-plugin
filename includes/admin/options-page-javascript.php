...
<script type="text/javascript">
    ...
    function <?= $this->slug_lc ?>DrugsOptions() {
        ...
        function addElement(element) {
            ...
            let label_for = '<?= $this->slug_lc ?>_field_list_drugs';
            let id_input = label_for + '_new_' + (new Date().getTime());
            let id_container = 'container_' + id_input;
            let data_cutom = 'custom';
            let name = '<?= $this->slug_lc ?>_options[' + label_for + '][]';
            html = html.replace(/%1\$./g, id_input).replace(/%2\$./g, data_cutom).replace(/%3\$./g, name).replace(/%4\$./g, "").replace(/%5\$./g, id_container).replace(/%6\$./g, '<?= $this->slug_lc ?>');
